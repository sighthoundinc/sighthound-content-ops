alter table public.blogs
  ...
  add column if not exists display_published_date date;
...
where scheduled_publish_date is null
  and target_publish_date is not null;

update public.blogs
set display_published_date = coalesce(
...
update public.blogs
set actual_published_at = coalesce(
  actual_published_at,
  published_at,
  status_updated_at,
  updated_at,
...
where actual_published_at is null
  and (
    overall_status = 'published'::public.overall_blog_status
    or publisher_status = 'completed'::public.publisher_stage_status
  );

alter table public.blogs enable trigger user;

create index if not exists blogs_actual_published_at_idx on public.blogs (actual_published_at);
create index if not exists blogs_display_published_date_idx on public.blogs (display_published_date);
