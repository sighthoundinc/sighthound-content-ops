...
! MUST be batched with #985 (accept-side delegation gap) on the SAME swarm
agent -- both fixes touch `scripts/triage_actions.py` and/or
`scripts/issue_ingest.py`, so the swarm Phase 1 Step 2 file-overlap audit
will force grouping anyway. Landing #988 without #985 leaves accepted
candidates not reaching `proposed/` at all, so the body-fidelity fix has
no users. Together they restore the full Phase 0 -> Phase 1 -> swarm
pipeline.
