...
## PDF Export Authority (MUST)
All paginated / printed exports across the app MUST route through the shared helper at `src/lib/pdf-export.ts`. Page-level HTML/CSS assembly for print is prohibited.
1. **Single source of truth**: `openBrandedPdfExport()` is the only sanctioned way to open a branded print popup. `renderBrandedPdfHtml()` is the pure function exported for tests and for `POST /api/exports/pdf`. `buildExportFilename()` is the only sanctioned way to produce an export filename.
2. **Brand chrome is fixed**: every export renders the Blurple→Red-Orange gradient stripe, the Sighthound logo (resolved as an absolute URL from `window.location.origin`), a title block (H1 Lexend 600 / 20px), a context block (exported by / timezone / scope / sort / active filters), a paginated `<table>` with a repeating `<thead>` and `page-break-inside: avoid` rows, and a footer with a `counter(page) / counter(pages)` page number. No per-page overrides.
3. **Brand tokens**: colors MUST come from `src/lib/print-brand-tokens.ts`. Hex literals for brand colors are not allowed in export code. Status chip tones use the `PdfStatusTone` union (`review`, `ready-to-publish`, `awaiting-live-link`, `changes-requested`, `in-progress`, `published`, `neutral`), which mirrors `src/lib/table-row-tones.ts`.
4. **Typography**: Lexend (300/400/500/600) is loaded via Google Fonts inside the detached popup. The host `@font-face` does not propagate to `window.open("", "_blank")`, so the helper embeds the link tags itself. Body 11px compact / 12px comfortable, H1 20px/600, meta 11–13px, uppercase table headers with letter-spacing 0.04em.
5. **Filename contract**: `content-relay_{surface}_{scope}_{YYYYMMDD-HHmm}_{tzAbbr}.{ext}`. Example: `content-relay_blogs_selected_20260421-0852_ET.pdf`. Timezone abbreviation mapping lives in `src/lib/pdf-export.ts`; unknown timezones fall back to a slugified IANA name.
6. **Context block requirements**: every export MUST populate the actor (`profile.full_name` + `profile.email`), timezone (respect the user preference, fallback `America/New_York`), scope (`view` or `selected`), sort (column label + direction), and any active filters as `{ label, value }` pairs. Omitting filters produces a visible `None` placeholder.
7. **Orientation + density**: `orientation: "landscape"` is appropriate when visible column count exceeds ~6 (Dashboard auto-switches). `density: "comfortable"` is opt-in per surface; default is `compact`. Both are options on `PdfExportOptions`; do not inline overrides in per-page CSS.
8. **Scope semantics**: `scope: "selected"` renders only currently selected rows. `scope: "view"` renders the currently filtered+sorted result set. Smart-scope helpers (`getSmartExportScope()`) on blogs/dashboard/social-posts default to `selected` when any rows are selected.
9. **Server-side rendering**: `POST /api/exports/pdf` accepts the same option shape and returns `text/html` for programmatic/cron exports. Rows are sanitized (strings and `{ kind: "chip" }` only); unknown shapes are dropped. Row cap: 5,000. Future Puppeteer/Playwright integration wraps this endpoint; do not re-implement the HTML.
10. **Covered surfaces**: Dashboard, Blogs, My Tasks, Social Posts list. Extending to a new surface requires: (a) calling `openBrandedPdfExport` with a `surface` slug, (b) registering filters + sort + actor, (c) extending `tests/pdf-export.test.ts` when behavior differs.
11. **Contract test**: `tests/pdf-export.test.ts` locks brand tokens, logo resolution, gradient stripe, Lexend loading, page counter, chip palette, landscape switch, HTML escaping, and filename pattern. Any branding/layout change MUST update both the helper and this test in the same change.
12. **No bypass**: page-level `window.open` + inlined HTML for export is a contract violation. Bug fixes and visual tweaks go into `src/lib/pdf-export.ts` — never into the caller.
...
16. **Canonical status labels.** `src/app/api/ai/utils/canonical-labels.ts` is the single source of truth for labels Gemini may use. The system prompt injects this allow-list; the output validator rejects raw enums (`ready_to_publish`, `canva_url`, etc.) and content-generation phrases.
17. **Output validator.** Every Gemini response passes through `validateGeminiOutput()` before the UI sees it. A rejection sets `validatorFailed=true` and causes the route to fall back to deterministic guidance (same as a Gemini outage).
18. **Response cache.** `/api/ai/assistant` uses an in-process LRU (`src/app/api/ai/utils/response-cache.ts`) keyed by `userId|entityType|entityId|updatedAt|normalizedPrompt` with a 5-minute TTL. Disabled when `ASK_AI_DISABLE_CACHE=true`. Workspace requests are NOT cached.
19. **Rate limiting.** Non-admin callers are limited to `ASK_AI_RATE_LIMIT_PER_MINUTE` (default 10) via `src/app/api/ai/utils/rate-limiter.ts`. Admins are exempt. 429 responses include a `Retry-After` header.
20. **Telemetry.** Every request writes a fire-and-forget row to `ai_assistant_events` via the service-role client (no prompt text, no answer text). Admins read aggregated metrics via `GET /api/admin/ai-health`.
21. **Feedback loop.** Users can submit thumbs up/down via `POST /api/ai/feedback`, stored in `ai_assistant_feedback`. RLS restricts users to their own rows; admins read everything.
22. **Safe-link curation.** Ask AI NEVER fabricates URLs. `src/app/api/ai/utils/safe-links.ts` derives a curated link list from grounded facts; Gemini may only reference these by their `key`. The API returns the resolved list as `data.links`.
23. **Assignee pill.** The API returns `data.assignee = { name, role } | null`, derived from RAG facts. UI surfaces this as a pill at the top of every response so the user knows who currently owns the record.
24. **Workspace scope.** `entityType = 'workspace'` supplies an aggregated read-only snapshot of a user's owned/assigned blogs and social posts (capped at 25 items). Workspace responses never reference or mutate individual items beyond pointing to them.
25. **Assistant surface.** `AIFloatingAssistant` is visible on content detail pages AND on workspace pages (`/dashboard`, `/tasks`, `/calendar`). Hidden elsewhere (Settings, Permissions, Activity History, etc.) per the existing "hide AI where it can't help" rule.
26. **Model routing.** Default model is `gemini-2.5-flash`; workspace queries with >10 items opt into `ASK_AI_COMPLEX_MODEL` when configured (e.g. `gemini-2.5-pro`).
