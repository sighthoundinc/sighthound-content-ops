...
export function OnboardingTour({ initiallyOpen }: { initiallyOpen?: boolean }) {
  ...
  return (
    <div
      ...
      className={cn(
        "fixed bottom-6 right-6 z-[260] w-[320px] overflow-hidden rounded-xl border border-[color:var(--sh-gray-200)] bg-white shadow-xl",
        "motion-reduce:transition-none transition-transform"
      )}
    >
      <div className="flex items-center justify-between border-b border-[color:var(--sh-gray)] px-4 py-2">
        ...
        <button
          type="button"
          onClick={finishTour}
          className="rounded p-1 text-navy-500 hover:bg-blurple-50"
      ...
      <div className="flex flex-col gap-3 px-4 py-3">
        <AppIcon
          ...
          boxClassName="h-10 w-10 rounded-lg bg-[color:var(--sh-gray)]"
          size={20}
        />
        <div>
          <p className="subsection-label text-ink">{step.title}</p>
          <p className="body-text mt-1 text-navy-500">{step.description}</p>
        </div>
