...
# ── Compare metric ───────────────────────────────────────────────────────────────────
# IMPORTANT: capture the python exit code directly. The previous form
# (RESULT=$(python3 ...) || IMPROVED=1; IMPROVED=$?) always set IMPROVED=0
# because $? read the exit status of the `IMPROVED=1` assignment, not python.
set +e
